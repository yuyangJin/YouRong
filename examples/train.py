...
def yr_main(rank, world_size, configs):
    ...
    print(model_name, seq_len)

    if model_name == "simple":
        model = MyModel(seq_len)
    else:
...
# def main():
    ...
    # log_queue = fthub.setup_primary_logging(filename)
    # logging.info(config)
    WORLD_SIZE = torch.cuda.device_count()
    print(WORLD_SIZE)
    mp.spawn(
        yr_main, args=(WORLD_SIZE, config), nprocs=WORLD_SIZE
    )
